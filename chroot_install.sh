...
# 挂载点检查与挂载
for MOUNT_POINT in /etc/resolv.conf /etc/hosts /sys /proc /dev; do
...
# 启动 QingLong
chroot $QL_INSTALL_DIR/$QL_BRANCH $SCRIPT_ENV -i HOME=/root TMPDIR=/tmp PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin TERM=linux /bin/bash <<'INNER_EOF'
...
#!/bin/bash
chroot $QL_INSTALL_DIR/$QL_BRANCH $SCRIPT_ENV -i /bin/bash <<'INNER_EOF'
  pm2 stop all
  pm2 flush
INNER_EOF
EOF
...
#!/bin/bash
chroot $QL_INSTALL_DIR/$QL_BRANCH $SCRIPT_ENV -i /bin/bash
EOF
